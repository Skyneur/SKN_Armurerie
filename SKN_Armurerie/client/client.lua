        ...
        CreateThread(function()
            while open do 
                RageUI.IsVisible(MenuArmurerie, function()
                    ...
                    RageUI.Separator("  ↓ _____________________ ~y~Accessoire _____________________   ~s~↓")
                    for k, v in pairs(Config.accessoire) do 
                        RageUI.Button(v.Nom .. '', nil, {RightLabel = "~g~" ..v.Price.." $"}, true, {
                            onSelected = function()
                                TriggerServerEvent('skn:Arme', v.Nom, v.Arme, v.Price)
                                RageUI.CloseAll()
                            end
                        })
                    end 

                    RageUI.Separator("  ↓ _____________________ ~o~Armes Blanche _____________________ ~s~↓")
                    ...
                    RageUI.Separator("  ↓ _____________________ ~r~Armes de point _____________________ ~s~↓")
    ...
    while true do
		local wait = 750
            ...
            if dist <= Config.MarkerDistance then
                wait = 0
                ESX.ShowHelpNotification("Appuyer sur [~b~E~s~] pour accéder à ~o~l'armurerie !")
...
RegisterNetEvent('skn:giveArme')
AddEventHandler('skn:giveArme', function(gun)
